...
(use-trait agent-account .aibtc-agent-account-traits.aibtc-account-config)

;; /g/SP2Z94F6QX847PMXTPJJ2ZCCN79JZDW3PJ4E6ZABY/faktory_attestor_deployer
(define-constant ATTESTOR_DEPLOYER 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM)
...
(define-map owner-to-agent-account
  principal
  principal
)
(define-map agent-to-agent-account
  principal
  principal
)
...
(define-public (auto-register-agent-account
    (owner principal)
    (agent principal)
  )
  (begin
...
(define-private (do-register-account
    (account principal)
    (owner principal)
    (agent principal)
  )
  (begin
    (asserts!
      (map-insert agent-account-registry account {
        owner: owner,
        agent: agent,
    ...
    (asserts! (map-insert owner-to-agent-account owner account)
      ERR_ALREADY_REGISTERED
    )
    (asserts! (map-insert agent-to-agent-account agent account)
      ERR_ALREADY_REGISTERED
    )
    (print {
...
(define-public (attest-agent-account (account principal))
  (let (
      (registry-entry (unwrap! (map-get? agent-account-registry account) ERR_ACCOUNT_NOT_FOUND))
    ...
    (asserts!
      (map-insert agent-account-attestations {
        account: account,
        attestor: tx-sender,
      }
        true
    ...
    (map-set agent-account-registry account
      (merge registry-entry { attestation-level: new-level })
    )
    (print {
...
(define-read-only (is-account-attested
    (account principal)
    (min-level uint)
  )
...
(define-read-only (has-attestor-signed
    (account principal)
    (attestor principal)
  )
  (default-to false
    (map-get? agent-account-attestations {
      account: account,
      attestor: attestor,
    })
  )
