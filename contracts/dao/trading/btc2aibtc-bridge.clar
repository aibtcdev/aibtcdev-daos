...
(define-constant OPERATOR_STYX 'SP33JZDSJ6B4WEGE0CQFEXGWB3QGMZ225HTV2S76E) ;; only 1 pool per operator else double spending 
        ...
        (match (get out (unwrap! (get-out-value wtx btc-receiver) ERR_NATIVE_FAILURE))
          out (if (>= (get value out) MIN_SATS)
            (let (
                ...
                (ai-account (unwrap! (contract-call? .register-ai-account get-ai-account-by-owner stx-receiver) ERR-NO-AI-ACCOUNT))
              )
