...
describe(`public functions: ${contractName}`, () => {
  ...
  it("buy() fails when market is closed", () => {
    ...
    const receipt = simnet.callPublicFn(
      contractAddress,
      "buy",
      [Cl.contractPrincipal(tokenContractAddress.split(".")[0], tokenContractAddress.split(".")[1]), Cl.uint(100000)],
      address1
    );
  ...
  it("buy() fails with non-positive sBTC amount", () => {
    ...
    const receipt = simnet.callPublicFn(
      contractAddress,
      "buy",
      [Cl.contractPrincipal(tokenContractAddress.split(".")[0], tokenContractAddress.split(".")[1]), Cl.uint(0)],
      address1
    );
  ...
  it("buy() succeeds with valid parameters", () => {
    ...
    if (initialTokenBalance.isOk && newTokenBalance.isOk) {
      expect(newTokenBalance.value.value).toBeGreaterThan(initialTokenBalance.value.value);
    }
  });
  ...
  it("sell() fails when market is closed", () => {
    ...
    const receipt = simnet.callPublicFn(
      contractAddress,
      "sell",
      [Cl.contractPrincipal(tokenContractAddress.split(".")[0], tokenContractAddress.split(".")[1]), Cl.uint(1000)],
      address1
    );
  ...
  it("sell() succeeds with valid parameters", () => {
    ...
    const receipt = simnet.callPublicFn(
      contractAddress,
      "sell",
      [Cl.contractPrincipal(tokenContractAddress.split(".")[0], tokenContractAddress.split(".")[1]), Cl.uint(sellAmount)],
      address1
    );
    ...
    if (initialSbtcBalance.isOk && newSbtcBalance.isOk) {
      expect(newSbtcBalance.value.value).toBeGreaterThan(initialSbtcBalance.value.value);
    }
  });
});
